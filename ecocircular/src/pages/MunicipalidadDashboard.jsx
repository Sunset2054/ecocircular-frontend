...
function GreenPointForm({ initialData, onSaved, token }) {
  const [form, setForm] = useState({
    ...
    acceptedMaterials: initialData?.acceptedMaterials?.map(m => m.id) || [],
  })
  ...
  const handleMaterialsChange = (e) => {
    ...
    setForm({ ...form, acceptedMaterials: options })
  }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <select multiple value={form.acceptedMaterials} onChange={handleMaterialsChange} style={{ height: 100 }}>
        {allMaterials.map(m => (
...
function MaterialForm({ initialData, onSaved, token }) {
  const [form, setForm] = useState({
    ...
    pointsPerKg: initialData?.pointsPerKg || 10,
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <TextField name="pointsPerKg" label="Puntos por kg" type="number" value={form.pointsPerKg} onChange={handleChange} />
